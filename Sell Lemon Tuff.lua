...
--  Modified with integrated smooth loading screen wrapper.
--==============================================================
...
-- HIDE MAIN GUI INITIALLY FOR LOADING SCREEN
main.Visible = false
...
local ab=Instance.new("Frame") ab.Size=UDim2.new(0,4,0,22) ab.Position=UDim2.new(0,12,0.5,-11) ab.BackgroundColor3=ACCENT ab.BorderSizePixel=0 bar.ZIndex=2 ab.Parent=bar corner(ab,2)
...
do
    ...
    local sw=Instance.new("TextButton") sw.Text="" sw.AutoButtonColor=false sw.Size=UDim2.new(0,42,0,22) sw.Position=UDim2.new(1,-42,0,7) sw.BackgroundColor3=OFFCOL sw.BorderSizePixel=0 sw.Parent=row corner(sw,11)
    local knob=Instance.new("Frame") knob.Size=UDim2.new(0,18,0,18) knob.Position=UDim2.new(0,2,0.5,-9) knob.BackgroundColor3=Color3.fromRGB(236,236,236) knob.BorderSizePixel=0 knob.Parent=sw corner(knob,9)
...
LoadFrame.Size = UDim2.new(0, 318, 0, 180)
LoadFrame.Position = UDim2.new(0.5, -159, 0.5, -90)
...
lstr.Color = Color3.fromRGB(0, 0, 0)
lstr.Transparency = 0.45
lstr.Thickness = 1.5
lstr.Parent = LoadFrame
...
LStatus.TextSize = 13
LStatus.TextColor3 = SUB
LStatus.Text = "Initializing script environment..."
LStatus.Parent = LoadFrame

local LTrack = Instance.new("Frame")
LTrack.Size = UDim2.new(0, 240, 0, 4)
LTrack.Position = UDim2.new(0.5, -120, 0, 110)
LTrack.BackgroundColor3 = BG3
LTrack.BorderSizePixel = 0
LTrack.Parent = LoadFrame
corner(LTrack, 2)

local LFill = Instance.new("Frame")
LFill.Size = UDim2.new(0, 0, 1, 0)
LFill.BackgroundColor3 = ACCENT
LFill.BorderSizePixel = 0
LFill.Parent = LTrack
corner(LFill, 2)

-- Loading Sequence Thread
task.spawn(function()
    local function updateStep(text, progress, dur)
        LStatus.Text = text
        TweenService:Create(LFill, TweenInfo.new(dur, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(progress, 0, 1, 0)}):Play()
        task.wait(dur)
    end

    updateStep("Fetching game modules...", 0.25, 0.6)
    updateStep("Bypassing secure remotes...", 0.55, 0.7)
    updateStep("Setting up orchard farm telemetry...", 0.85, 0.5)
    updateStep("Ready!", 1.0, 0.3)

    -- Fade out the loading elements cleanly
    TweenService:Create(LoadFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {BackgroundTransparency = 1}):Play()
    TweenService:Create(LTitle, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {TextTransparency = 1}):Play()
    TweenService:Create(LStatus, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {TextTransparency = 1}):Play()
    TweenService:Create(LTrack, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {BackgroundTransparency = 1}):Play()
    TweenService:Create(LFill, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {BackgroundTransparency = 1}):Play()
    TweenService:Create(lstr, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Transparency = 1}):Play()
    task.wait(0.3)
    
    LoadFrame:Destroy()
    main.Visible = true -- Reveal the core UI window
end)
...
print("[Sell Lemons Farm] v4 loaded with smooth loader UI")
